...
function Navbar(props) {
    ...
    const navbarLinks = links.map((link) => (
        <div key={link.name}>
            <NavLink to={link.path} className="navbar-item">
                {link.name}
            </NavLink>
        </div>
    ));
    ...
    return (
        <>
            <header className={"nav-header " + (scrollPosY > 15 ? "navbar-underline" : "navbar-underline-inactive")}>
                ...
                {/* <p style={{justifySelf: "center", paddingLeft: "24px"}}>Robert Tanzola</p> */}
